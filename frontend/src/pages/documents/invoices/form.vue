...
import { fetchStaticData } from '@/services/api/invoice';
import type { DocumentItems, Invoices } from '@/services/models';
import { documentCoreDefaults, getDefaultInvoices } from '@services/defaults';

const store = useDocumentCoreStore();
const emit = defineEmits(['close', 'submit', 'save']);
const invoiceFormDefaults = ref<Invoices>(store.selectedDocumentCore || getDefaultInvoices());
...
const invoiceData = ref<Invoices>(getDefaultInvoices())
...
watch(() => store.selectedDocumentCore, (newinvoice) => {
  if (store.mode === 'edit' && newinvoice) {
    invoiceFormDefaults.value = { ...newinvoice };
  }
}, { immediate: true });

onMounted(async () => {
  if (store.mode === 'add') {
    invoiceFormDefaults.value = getDefaultInvoices();
  } else if (store.mode === 'edit' && store.selectedDocumentCore) {
    invoiceFormDefaults.value = { ...store.selectedDocumentCore };
...
const addProduct = (value: DocumentItems) => {
  if (invoiceData.value?.documentsItems && store.mode === 'add') {
    value.order = (invoiceData.value.documentsItems.length > 0 ? Math.max(...invoiceData.value.documentsItems.map(item => item.order ?? 0)) : 0) + 1;
    orderItems.value = value.order
    invoiceData.value.documentsItems.push(value);
  }
...
const removeProduct = (id: number) => {
  invoiceData.value?.documentsItems.splice(id, 1)
}
...
<template>
  ...
  <VRow>
    <!-- 👉 InvoiceEditable -->
    <VCol cols="12" md="9">
      <DocumentsEditable documentType='Facture'  :data="invoiceData" @push="addProduct" @remove="removeProduct" :triggerSubmit="shouldSubmit"
    ...
    <VCol cols="12" md="3">
      <VCard class="mb-8">
        <VCardText>
          <!-- 👉 Save -->
          <VBtn block color="primary" class="mb-4" @click="submitinvoice">
            Enregistrer
          </VBtn>
