...
import type { DocumentItems, Refunds } from '@/services/models';
import { documentCoreDefaults, getDefaultRefunds } from '@services/defaults';


const store = useDocumentCoreStore();
const emit = defineEmits(['close', 'submit', 'save']);
const creditsFormDefaults = ref<Refunds>(store.selectedDocumentCore || getDefaultRefunds());
...
const creditData = ref<Refunds>(getDefaultRefunds())
...
onMounted(async () => {
  if (store.mode === 'add') {
    creditsFormDefaults.value = getDefaultRefunds();
...
<template>
  ...
  <VRow>
    ...
    <VCol cols="12" md="3">
      ...
      <!-- 👉 Select Status-->
      <!-- <VSelect v-if="documentsStore.mode === 'edit'" v-model="documentsFormDefaults.status" :items="status"
      :item-props="getItemProps" 
        variant="outlined" label="Statut" placeholder="Statut" class="mb-6" /> -->

      <!-- 👉 Taxable -->
      <!-- <div class="d-flex align-center justify-space-between">
        <VLabel>Tax</VLabel>
        <div>
          <VSwitch @click="isTaxableVal()" v-model="isTaxableComputed" />
        </div>
      </div> -->
