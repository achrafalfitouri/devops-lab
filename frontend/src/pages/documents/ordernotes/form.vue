...
import type { DocumentItems, OrderNotes } from '@/services/models';
import { documentCoreDefaults, getDefaultOrderNotes } from '@services/defaults';

const store = useDocumentCoreStore();
const emit = defineEmits(['close', 'submit', 'save']);
const ordernoteFormDefaults = ref<OrderNotes>(store.selectedDocumentCore || getDefaultOrderNotes());
...
const ordernoteData = ref<OrderNotes>(getDefaultOrderNotes())
...
watch(() => store.selectedDocumentCore, (newOrdernote) => {
  if (store.mode === 'edit' && newOrdernote) {
    ordernoteFormDefaults.value = { ...newOrdernote };
  }
}, { immediate: true });

onMounted(async () => {
  if (store.mode === 'add') {
    ordernoteFormDefaults.value = getDefaultOrderNotes();
  } else if (store.mode === 'edit' && store.selectedDocumentCore) {
    ordernoteFormDefaults.value = { ...store.selectedDocumentCore };
...
const addProduct = (value: DocumentItems) => {
  if (ordernoteData.value?.documentsItems && store.mode === 'add') {
    value.order = (ordernoteData.value.documentsItems.length > 0 ? Math.max(...ordernoteData.value.documentsItems.map(item => item.order ?? 0)) : 0) + 1;
    orderItems.value = value.order
    ordernoteData.value.documentsItems.push(value);
  }
...
const removeProduct = (id: number) => {
  ordernoteData.value?.documentsItems.splice(id, 1)
}
...
<template>
  ...
  <VRow>
    <!-- 👉 InvoiceEditable -->
    <VCol cols="12" md="9">
      <DocumentsEditable documentType='Bon de commande'  :data="ordernoteData" @push="addProduct" @remove="removeProduct" :triggerSubmit="shouldSubmit"
