...
import type { DocumentItems, Quotes } from '@/services/models';
import { useDocumentCoreStore } from '@/stores/documents';
import { documentCoreDefaults, getDefaultQuotes } from '@services/defaults';
import { ref, watch } from 'vue';

const store = useDocumentCoreStore();
const emit = defineEmits(['close', 'submit', 'save']);
const quoteFormDefaults = ref<Quotes>(store.selectedDocumentCore || getDefaultQuotes());
...
const quoteData = ref<Quotes>(getDefaultQuotes())
...
onMounted(async () => {
  if (store.mode === 'add') {
    quoteFormDefaults.value = getDefaultQuotes();
...
<template>
  ...
  <VRow>
    <!-- 👉 InvoiceEditable -->
    <VCol cols="12" md="9">
      <DocumentsEditable documentType='Devis' :data="quoteData" @push="addProduct" :triggerSubmit="shouldSubmit"
