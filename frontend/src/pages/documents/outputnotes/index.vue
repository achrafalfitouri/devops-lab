...
import { deleteOutputNotes, exportOutputNotes, fetchArchivedOutputNotes, fetchOutputNotes, fetchOutputNotesById, fetchStaticData } from '@/services/api/outputnote';
import { useArchiveStoreStore } from '@/stores/archive';
import { useAuthStore } from '@/stores/auth';
import { useDocumentCoreStore } from '@/stores/documents';
import { getDefaultOutputNotes, getDefaultOutputNotesFilterParams } from '@services/defaults';
import type { OutputNotes, OutputNotesFilterParms } from '@services/models';
...
const isAddNewoutputnotesDrawerVisible = ref(false)

// 👉 State variables for form validation and loading status
const isLoading = ref(true)

// 👉 State variable for the selected outputnotes in edit mode
const selectedoutputnotes = ref<OutputNotes>(getDefaultOutputNotes())

// 👉 State for dialog visibility
const isDeleteDialogVisible = ref(false)

// 👉 State for storing the outputnotes to delete
const outputnotesToDelete = ref<OutputNotes | null>(null)
...
// 👉 Filters and search
const filterParams = ref<OutputNotesFilterParms>(getDefaultOutputNotesFilterParams())
...
// 👉 State variable for storing outputnotess data
const outputnotesData = ref<{
  outputnotes: OutputNotes[]
  totaloutputnotes: number
}>({
  outputnotes: [],
  totaloutputnotes: 0,
})

// 👉 Function to fetch outputnotess from the API
const loadoutputnotes = async () => {
  ...
  try {
    ...
    if (archiveStore.isArchive) {
      ...
      data = await fetchArchivedOutputNotes(filters, queryParams.value.itemsPerPage, queryParams.value.page);
    }
    else {
      data = await fetchOutputNotes(filters, queryParams.value.itemsPerPage, queryParams.value.page);
    }
    outputnotesData.value.outputnotes = data.outputNotes.data || [];
    outputnotesData.value.totaloutputnotes = data.totalOutputNotes || 0;
    if (outputnotesData.value.outputnotes.length === 0 && queryParams.value.page > 1) {

      queryParams.value.page--;
      await loadoutputnotes();
    }
...
const exportOutputNote = async () => {
  ...
  try {
    ...
    const response = await exportOutputNotes(filters, queryParams.value.itemsPerPage, queryParams.value.page);

    if (response.status === 200 && response.downloadUrl) {
      const downloadUrl = response.downloadUrl;
      const link = document.createElement('a');
      link.href = downloadUrl;
      link.setAttribute('download', 'outputnotes.xlsx');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else {
      console.error('Failed to export outputnotes: Invalid response format');
    }
  } catch (error) {
...
// 👉 Replace throttledLoadOutputNotess with debouncedLoadOutputNotess
const debouncedLoadOutputNotess = debounce(async () => {
  await loadoutputnotes();
}, 800);

// 👉 Watchers to reload outputnotess on filter, search, pagination, or sorting change
watch(filterParams, () => {
  debouncedLoadOutputNotess();
}, { deep: true });
...
const onSearchInput = (newSearch: string) => {
  if (newSearch) {
    filterParams.value = {
      ...getDefaultOutputNotesFilterParams(),
...
const openView = (outputNotes: OutputNotes) => {
  store.setEditMode(outputNotes);
  store.setPreviewMode();

}

// 👉 Opens the drawer for editing a outputnotes.
const openEditoutputnotesDrawer = async (outputnotes: OutputNotes) => {
  const id = outputnotes.id
  const data = await fetchOutputNotesById(id)
  store.setEditMode(data);
  await router.push('/documents/outputnotes/form');

}

// 👉 Function to open the delete confirmation dialog
const openDeleteDialog = (outputnotes: OutputNotes) => {
  outputnotesToDelete.value = outputnotes
  isDeleteDialogVisible.value = true
}

// 👉 Function to handle delete action
const confirmDeleteoutputnotes = async () => {
  const outputnotesId = outputnotesToDelete.value?.id
  if (!outputnotesId) {
    console.error('outputnotes ID is undefined. Cannot delete outputnotes.')
    return
  }
  try {
    await deleteOutputNotes(outputnotesId)
    isDeleteDialogVisible.value = false
    await loadoutputnotes()
    showSnackbar('le bon de sortie', 'success');
  } catch (error) {
...
const cancelDelete = () => {
  isDeleteDialogVisible.value = false
  outputnotesToDelete.value = null
}
...
<template>
  <section>
    ...
    <VCard class="mb-6">
      ...
      <VCardText>
        <VRow align="center" class="justify-end">
          ...
          <VCol class="d-flex align-center gap-2 justify-end">
            ...
            <!-- 👉 Export button -->
            <VBtn v-if="!archiveStore.isArchive" @click="exportOutputNote" variant="tonal" color="secondary"
      ...
      <VCardText>
        <VRow>
          ...
          <VCol cols="12" md="4">
            <VAutocomplete v-model="filterParams.selectedStatus" :items="outputNoteStatus"
    ...
    <!-- 👉 Data Table -->
    <VCard>
      <VDataTable :headers="headers" :items="outputnotesData.outputnotes"
        ...
        <template #item.code="{ item }">
          ...
          <RouterLink
            :to="{ name: archiveStore.isArchive ? 'archives-documents-outputnotes-id' : 'documents-outputnotes-id', params: { id: item.id } }"
        ...
        <template #item.actions="{ item }">
          <div style="display: flex; gap: 8px;">
            ...
            <IconBtn v-if="hasDocumentManagerPermission && !archiveStore.isArchive"
              ...
              @click="openEditoutputnotesDrawer(item)">
        ...
        <template #bottom>
          <TablePagination :page="queryParams.page" :items-per-page="queryParams.itemsPerPage"
            :totalItems="outputnotesData.totaloutputnotes" @update:page="onPageChange" />
        </template>

      </VDataTable>
    </VCard>
    <!-- Delete Confirmation Dialog -->
    <AlertDialog :isVisible="isDeleteDialogVisible" message="Êtes-vous sûr de vouloir archiver ce bon de sortie?"
      deleteButtonText="Confirmer" cancelButtonText="Annuler" deleteButtonColor="error" cancelButtonColor="secondary"
      :onDeleteClick="confirmDeleteoutputnotes" :onCancelClick="cancelDelete"
