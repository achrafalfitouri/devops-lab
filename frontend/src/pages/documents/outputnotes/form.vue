...
import { fetchStaticData } from '@/services/api/outputnote';
import type { DocumentItems, OutputNotes } from '@/services/models';
import { documentCoreDefaults, getDefaultOutputNotes } from '@services/defaults';


const store = useDocumentCoreStore();
const emit = defineEmits(['close', 'submit', 'save']);
const outputnoteFormDefaults = ref<OutputNotes>(store.selectedDocumentCore || getDefaultOutputNotes());
...
const outputnoteData = ref<OutputNotes>(getDefaultOutputNotes())
...
watch(() => store.selectedDocumentCore, (newOutputnote) => {
  if (store.mode === 'edit' && newOutputnote) {
    outputnoteFormDefaults.value = { ...newOutputnote };
  }
}, { immediate: true });

onMounted(async () => {
  if (store.mode === 'add') {
    outputnoteFormDefaults.value = getDefaultOutputNotes();
  } else if (store.mode === 'edit' && store.selectedDocumentCore) {
    outputnoteFormDefaults.value = { ...store.selectedDocumentCore };
...
const addProduct = (value: DocumentItems) => {
  if (outputnoteData.value?.documentsItems && store.mode === 'add') {
    value.order = (outputnoteData.value.documentsItems.length > 0 ? Math.max(...outputnoteData.value.documentsItems.map(item => item.order ?? 0)) : 0) + 1;
    orderItems.value = value.order
    outputnoteData.value.documentsItems.push(value);
  }
...
const removeProduct = (id: number) => {
  outputnoteData.value?.documentsItems.splice(id, 1)
}

const submitOutputnote = () => {
...
<template>
  ...
  <VRow>
    <!-- 👉 InvoiceEditable -->
    <VCol cols="12" md="9">
      <DocumentsEditable documentType='Bon de sortie'  :data="outputnoteData" @push="addProduct" @remove="removeProduct" :triggerSubmit="shouldSubmit"
    ...
    <VCol cols="12" md="3">
      <VCard class="mb-8">
        <VCardText>
          <!-- 👉 Save -->
          <VBtn block color="primary" class="mb-4" @click="submitOutputnote">
            Enregistrer
          </VBtn>
