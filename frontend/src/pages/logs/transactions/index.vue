...
import { fetchTransactionLogss } from '@/services/api/transactionlogs';
...
// 👉 State variable for storing transactionlogss data
const transactionlogssData = ref<{
  transactionlogss: Logs[]
  totalTransactionLogss: number
}>({
  transactionlogss: [],
  totalTransactionLogss: 0,
})

// 👉 Function to fetch transactionlogss from the API
const loadTransactionLogss = async () => {
  ...
  try {
    ...
    const data = await fetchTransactionLogss(filters, queryParams.value.itemsPerPage, queryParams.value.page);
    transactionlogssData.value.transactionlogss = data.transactionLogs || [];
    transactionlogssData.value.totalTransactionLogss = data.totalTransactionLogs || 0;
    if (transactionlogssData.value.transactionlogss.length === 0 && queryParams.value.page > 1) {

      queryParams.value.page--;
      await loadTransactionLogss();
    }
...
onMounted(() => {
  loadTransactionLogss()
});

// Replace throttledLoadTransactionLogss with debouncedLoadTransactionLogss
const debouncedLoadTransactionLogss = debounce(async () => {
  await loadTransactionLogss();
}, 800);

// 👉 Watchers to reload transactionlogss on filter, search, pagination, or sorting change
watch(filterParams, () => {
  debouncedLoadTransactionLogss();
}, { deep: true });
...
<template>
  <section>
    ...
    <!-- 👉 Data Table -->
    <VCard>
      <VDataTable v-model:items-per-page="queryParams.itemsPerPage" :items="transactionlogssData.transactionlogss"
        ...
        <template #item.oldValue="{ item }">
          <RouterLink :to="{ name: 'logs-transactions-id', params: { id: item.id } }"
        ...
        <template #bottom>
          <TablePagination :page="queryParams.page" :items-per-page="queryParams.itemsPerPage"
            :totalItems="transactionlogssData.totalTransactionLogss" @update:page="onPageChange" locale="fr" />
        </template>
      </VDataTable>
    </VCard>
