...
import { fetchPaymentLogss, fetchRecoveryLogs } from '@/services/api/paymentlogs';
...
// 👉 State variable for storing paymentlogss data
const paymentlogssData = ref<{
  paymentlogss: Logs[]
  totalPaymentLogss: number
}>({
  paymentlogss: [],
  totalPaymentLogss: 0,
})

// 👉 State variable for storing recoverylogss data
const recoverylogssData = ref<{
  recoverylogss: Logs[]
  totalRecoveryLogss: number
}>({
  recoverylogss: [],
  totalRecoveryLogss: 0,
})

// 👉 Function to fetch paymentlogss from the API
const loadPaymentLogss = async () => {
  ...
  try {
    ...
    const [data, data2] = await Promise.all([
      fetchPaymentLogss(filters, queryParams.value.itemsPerPage, queryParams.value.page),
      fetchRecoveryLogs(filters, queryParams.value.itemsPerPage, queryParams.value.page)
    ]);
    paymentlogssData.value.paymentlogss = data.paymentLogs || [];
    paymentlogssData.value.totalPaymentLogss = data.totalPaymentLogs || 0;
    recoverylogssData.value.recoverylogss = data2.recoveryLogs || [];
    recoverylogssData.value.totalRecoveryLogss = data2.totalRecoveryLogs || 0;
    if (paymentlogssData.value.paymentlogss.length === 0 && queryParams.value.page > 1) {

      queryParams.value.page--;
      await loadPaymentLogss();
    }
    if (recoverylogssData.value.recoverylogss.length === 0 && queryParams.value.page > 1) {
...
// Replace throttledLoadPaymentLogss with debouncedLoadPaymentLogss
const debouncedLoadPaymentLogss = debounce(async () => {
  await loadPaymentLogss();
}, 800);

// 👉 Watchers to reload paymentlogss on filter, search, pagination, or sorting change
watch(filterParams, () => {
  debouncedLoadPaymentLogss();
}, { deep: true });
...
<template>
  <section>
    ...
    <VCard>
      <VCol cols="12" md="4">
        <VCardTitle class="pa-0">Logs des paiements</VCardTitle>
      </VCol>
      <VDataTable v-model:items-per-page="queryParams.itemsPerPage" :items="paymentlogssData.paymentlogss"
        ...
        <template #item.oldValue="{ item }">
          <RouterLink :to="{ name: 'logs-payments-id', params: { id: item.id } }"
        ...
        <template #bottom>
          <TablePagination :page="queryParams.page" :items-per-page="queryParams.itemsPerPage"
            :totalItems="paymentlogssData.totalPaymentLogss" @update:page="onPageChange" locale="fr" />
        </template>
      </VDataTable>
    </VCard>
    ...
    <VCard>
      <VCol cols="12" md="4">
        <VCardTitle class="pa-0">Logs des recouvrements</VCardTitle>
      </VCol>
      <VDataTable v-model:items-per-page="queryParams.itemsPerPage" :items="recoverylogssData.recoverylogss"
        :headers="headers" item-value="id" class="text-no-wrap" :loading="isLoading" locale="fr">
        ...
        <template #item.oldValue="{ item }">
          <RouterLink :to="{ name: 'logs-payments-id', params: { id: item.id }, query: { type: 'recovery' } }"
            class="d-flex align-center gap-1 text-decoration-none" style="color: inherit;">
            <span
              style="display: inline-block; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
              {{ item.oldValue }}
        ...
        <template #bottom>
          <TablePagination :page="queryParams.page" :items-per-page="queryParams.itemsPerPage"
            :totalItems="recoverylogssData.totalRecoveryLogss" @update:page="onPageChange" locale="fr" />
        </template>
      </VDataTable>
    </VCard>
