...
export const isNavLinkActive = (link: NavLink, router: Router) => {
  ...
  let resolvedLink = typeof link.to === 'string' ? link.to : router.resolve(link.to).path;

  if (!resolvedLink.startsWith('/')) {
    resolvedLink = '/' + resolvedLink;
  }
  
  resolvedLink = resolvedLink.replace(/-/g, '/');


  if (currentRoute.path === resolvedLink) {
    return true;
  }

  if (resolvedLink.startsWith('/documents/quotes')) {
    const isActive = currentRoute.path.startsWith('/documents/quotes/');
    return isActive;
  }

  const isActive = currentRoute.path.startsWith(resolvedLink);
  return isActive;
};
 ...
 */
export function isNavGroupActive(children: NavGroup['children'], router: Router): boolean {
  const currentPath = router.currentRoute.value.fullPath;

  return children.some(child => {
    const formattedTo = typeof child.to === 'string' ? `/${child.to.replace(/-/g, '/')}` : '';

    const isActive = currentPath.startsWith(formattedTo);

    if ('children' in child && Array.isArray(child.children)) {
      return isActive || isNavGroupActive(child.children, router);
    }
