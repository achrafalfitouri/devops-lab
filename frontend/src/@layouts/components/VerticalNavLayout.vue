...
export default defineComponent({
  ...
  return props.navItems.filter(item => {
    // Check if item has a 'title' property (i.e., it's not a 'NavSectionTitle')
    if ('title' in item) {
      const permissionConfig = getRequiredPermissionsForNavItem(item.title);

      // Use the helper function to check permissions
      return hasRequiredPermissions(authStore.permissions, permissionConfig);
    }

    // If item is a NavSectionTitle or other type without a 'title', include it by default
    return true;
  });
});
    ...
    return () => {
      ...
      const verticalNav = h(
        VerticalNav,
        { isOverlayNavActive: isOverlayNavActive.value, toggleIsOverlayNavActive, navItems: filteredNavItems.value, ...additionalVerticalNavAttrs },
        {
          'nav-header': () => slots['vertical-nav-header']?.(),
          'before-nav-items': () => slots['before-vertical-nav-items']?.(),
        },
      )
      const showNavbarOnMobile = computed(() => windowWidth.value < 1280)

      // 👉 Navbar, only rendered when on mobile
      const navbar = showNavbarOnMobile.value
        ? h(
          'header',
...
<style lang="scss">
.highlight-profile {
  background-color: #f0f8ff; // Light background color for highlighting
  color: #007bff; // Text color for the highlighted item
