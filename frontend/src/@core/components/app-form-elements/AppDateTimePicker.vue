...
const createYearDropdown = (fp: any) => {
  return {
    onReady: () => {
      const yearInput = fp.currentYearElement;
      if (!yearInput) return;

      // Inject modern style for select and options
      const styleTag = document.createElement('style');
      styleTag.textContent = `
        select.custom-year-dropdown::-webkit-scrollbar {
          display: none;
        }

        select.custom-year-dropdown option:checked {
          background-color: rgba(98, 0, 238, 0.15); /* example of secondary tone (adjust to your theme) */
          color: black;
        }

        select.custom-year-dropdown option {
      ...
      yearSelect.className = `${yearInput.className} custom-year-dropdown`;
      ...
      yearSelect.addEventListener('change', (e) => {
        ...
        fp.changeYear(parseInt(target.value));
      });

      // Hover effects
      yearSelect.addEventListener('mouseenter', () => {
        yearSelect.style.backgroundColor = 'rgba(var(--v-theme-secondary), 0.08)';
      });

      yearSelect.addEventListener('mouseleave', () => {
      ...
      // Replace original year input
      yearInput.parentNode?.replaceChild(yearSelect, yearInput);
      fp.currentYearElement = yearSelect;
    },

    onYearChange: () => {
      if (fp.currentYearElement && fp.currentYearElement.tagName === 'SELECT') {
        fp.currentYearElement.value = fp.currentYear.toString();
      }
    }
  };
};
          ...
          <template #default="{ props: vFieldProps }">
            <div v-bind="vFieldProps">
              ...
              <FlatPickr v-if="!isInlinePicker" v-bind="compAttrs" ref="refFlatPicker" :model-value="modelValue"
                ...
                class="flat-picker-custom-style h-100 w-100" :disabled="isReadonly.value" @on-open="onPickerOpen"
                @on-close="onPickerClose" @update:model-value="emitModelValue" />
